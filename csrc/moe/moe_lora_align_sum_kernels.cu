...
// Global memory fallback version for models with many experts
template <typename scalar_t, typename token_cnts_t>
__global__ void moe_lora_align_sum_kernel_global(
    ...
    int32_t* __restrict__ sorted_token_ids, int32_t* __restrict__ expert_ids,
    int topk_num, int32_t* total_tokens_post_pad, int32_t* adapter_enabled,
    int32_t* lora_ids, int32_t* global_cumsum, token_cnts_t* global_tokens_cnts) {
  const size_t tokens_per_thread = div_ceil(numel, blockDim.x);
  const size_t start_idx = threadIdx.x * tokens_per_thread;
  ...
  token_cnts_t* tokens_cnts = global_tokens_cnts + 
                              lora_idx * (blockDim.x + 1) * num_experts;
...
void moe_lora_align_block_size(
  ...
  const bool use_global_memory = (shared_mem > device_max_shared_mem);
  ...
  VLLM_DISPATCH_INTEGRAL_TYPES(
      topk_ids.scalar_type(), "moe_lora_align_sum_kernel", [&] {
        ...
        if (use_global_memory) {
          // Use global memory fallback for models with many experts
          // Allocate global memory buffers
          auto options = torch::TensorOptions()
                          .dtype(torch::kInt32)
                          .device(topk_ids.device());
          
          // cumsum buffer: max_loras * (num_experts + 1)
          torch::Tensor global_cumsum = torch::empty(
              {max_loras, num_experts + 1}, options);
          
          // tokens_cnts buffer: max_loras * (num_thread + 1) * num_experts
          torch::Tensor global_tokens_cnts = torch::empty(
              {max_loras, num_thread + 1, num_experts}, options);
          ...
          // CRITICAL: Synchronize to ensure kernel completes before tensors
          // go out of scope and their memory is freed
          cudaStreamSynchronize(stream);
        } else {
          // Use shared memory (original implementation)
